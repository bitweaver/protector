<?php
/**
 * @version $Header: /cvsroot/bitweaver/_bit_protector/LibertyProtector.php,v 1.5 2006/11/01 10:06:06 lsces Exp $
 *
 ...
 *
 * $Id: LibertyProtector.php,v 1.5 2006/11/01 10:06:06 lsces Exp $
 * @package protector
 */
 ...
 *
 * @version $Revision: 1.5 $ $Date: 2006/11/01 10:06:06 $ $Author: lsces $
 */
...
function protector_content_list() {
	...
	$ret = array(
		'join_sql' => " LEFT JOIN `".BIT_DB_PREFIX."liberty_content_group_map` lcgm ON ( lc.`content_id`=lcgm.`content_id` ) LEFT OUTER JOIN `".BIT_DB_PREFIX."users_groups_map` ugm ON ( ugm.`user_id`=? ) AND ( ugm.`group_id`=lcgm.`group_id` ) ",
		'where_sql' => " AND (lcgm.`content_id` IS NULL OR lcgm.`group_id` IN(". implode(',', array_fill(0, count($groups), '?')) ." ) OR ugm.`user_id`=?) ",
		'bind_vars' => array_merge( array( $gBitUser->mUserId ), $groups, array( $gBitUser->mUserId ) ),
	);
